...
class Product:
    def __init__(
        self, product_name, products, path, page_address, products_category, tshirt=True
    ):
        ...
        self.product_category = self.get_product_category(
            products_category, products[-1]
        )
    ...
    def set_product(self, bs, page_address, path, product_category):
        ...
        self.product["sub_name"] = bs.find("h1", id="productTitle").text
        self.product["subproducts"] = self.products
        self.product["id"] = bs.find(
            "span", class_="product__desc-number--bold"
        ).get_text()
        self.product["price"] = self.get_price(bs)
        self.product["weight"] = (
            bs.find("p", class_="product__availability-item").find("span").get_text()
        )
        self.product["availability"] = self.get_availability(bs)
        self.product["expected_availability"] = bs.find_all(
            "p", class_="product__availability-item"
        ...
        self.product["image_link"] = self.get_first_image_link(
            links_images=self.product["images"]
        )
        self.product["brand"] = "VW"
        self.product["additional_image_link"] = self.additional_image_link(
            links_images=self.product["images"]
        )
        ...
        self.product["custom_label_0"] = (
            product_category["last_category"].replace(" ", "-")
            if product_category["last_category"] is not None
            else None
        )
    ...
    def get_availability(self, bs):
        availability = (
            bs.find_all("p", class_="product__availability-item")[1]
            .find("span")
            .get_text()
        )
    ...
    def get_description(self, bs):
        description = bs.find("div", class_="product__txt-desc").get_text(
            "|", strip=True
        )
